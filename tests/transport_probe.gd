...
func _run() -> void:
	...
	check(
		guests[0].join_steam("UP4-123") == ERR_ALREADY_IN_USE, "invite preserves active connection"
	)
	for guest in guests:
	...
	for code in ["invalid", "UP1-123-token", "UP2-123", "UP3-123", "UP4-0"]:
		check(
...
func _steam_room_check() -> void:
	...
	for _attempt in range(100):
		if host_node.invite_ready() or not host_node.is_host:
			break
		await pause(0.2)
	check(host_node.invite_ready() and host_node.room_code.begins_with("UP4-"), "Steam room ready")
